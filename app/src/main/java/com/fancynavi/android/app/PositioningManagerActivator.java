...
class PositioningManagerActivator {
    ...
    PositioningManagerActivator(PositioningManager.LocationMethod locationMethod, boolean activateHereAdvancedPositioning) {
        ...
        Snackbar.make(DataHolder.getActivity().findViewById(R.id.mapFragmentView), DataHolder.getAndroidXMapFragment().getString(R.string.positioning) + positioningManager.getDataSource().getLocationSource().name() + " / " + positioningManager.getLocationMethod(), Snackbar.LENGTH_SHORT).show();
